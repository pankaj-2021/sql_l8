...
select dept, sum(salary) as totalemsalary from employees group by dept;
...
select dept,count(name) as countemployees from employees group by dept order by countemployees desc ;
...
SELECT  Dept, count(name) as empcount FROM Employees group by  Dept having count(name)>7;
...
select dept,count(name) as femalecount from employees where gender='f' group by dept;
...
SELECT Dept, City, COUNT(name) as EmpCount FROM Employees GROUP BY Dept, City order by dept
...
SELECT dept, Role, count(name) as empcount FROM Employees GROUP BY dept, Role having empcount>1 ;
...
SELECT city,dept,count(name) as empcount from employees group by city,dept having empcount>1 order by empcount desc


select role, sum(salary) as totalsalary from employees group by role having sum(salary)>300000 order by totalsalary asc;

SELECT UPPER(Name) as EmpName, YEAR(JoinDate) as JoinYear FROM Employees order by joinyear asc;

SELECT YEAR(JoinDate) as JoinYear, COUNT(name) as Joiners FROM Employees GROUP BY
YEAR(JoinDate) ORDER BY JoinYear asc;
